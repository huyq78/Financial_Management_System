...
async function init () {
    ...
    await sleep(1000)
    node.innerText = ""
    await node.type('Your expenditure is so huge, uncontrolled?')
    while(true) {
      await sleep(2000)
      await node.delete('Your expenditure is so huge, uncontrolled?')
      await node.type('You want to track your spending?')
      await sleep(2000)
      await node.delete('You want to track your spending?')
      await node.type('Click here 👇🏻')
      await sleep(2000)
      await node.delete('Click here 👇🏻')
    }
  }
